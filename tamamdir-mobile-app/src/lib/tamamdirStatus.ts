...
export function getCancelModalCopy(status: TamamdirStatus | null) {
  if (!status?.is_customer) {
    return {
      ...
      note: "Sağlayıcılar iptal nedeniyle banlanmaz.",
    };
  }
  ...
  if (status.will_be_banned_if_cancel) {
    return {
      body: "Bu hizmet anlaşmasını iptal etmek istediğine emin misin?",
      note: `Müşteriler en fazla ${limit} kez iptal edebilir (${used}/${limit} kullanıldı). Bu son iptalin — 1 hafta banlanırsın.`,
    };
  }
  ...
  return {
    body: "Bu hizmet anlaşmasını iptal etmek istediğine emin misin?",
    note: `Müşteriler en fazla ${limit} kez iptal edebilir (${used}/${limit} kullanıldı). Banlanmadan önce ${leftAfter} iptal hakkın kalır.`,
  };
}
